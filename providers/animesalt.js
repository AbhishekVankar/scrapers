...
function parseM3u8Qualities(m3u8Text, baseUrl) {
  var qualities = []
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (line.indexOf('#EXT-X-STREAM-INF') === 0) {
      ...
      var quality = height >= 1080 ? '1080p' : height >= 720 ? '720p' : height >= 480 ? '480p' : '360p'
      var next = lines[i + 1] ? lines[i + 1].trim() : ''
      if (next && next.indexOf('#') !== 0) {
        var streamUrl = next.indexOf('http') === 0 ? next : baseUrl + '/' + next
        qualities.push({ quality: quality, url: streamUrl })
        i++
      }
    }
  }
  ...
  return httpGet(pageUrl, { 'Referer': BASE + '/' })
    .then(function(html) {
      ...
      return httpPost(
        ...
        var m3u8Base = m3u8.substring(0, m3u8.lastIndexOf('/'))
        return httpGet(m3u8, { 'Referer': cdnBase + '/' })
          .then(function(m3u8Text) {
            var qualities = parseM3u8Qualities(m3u8Text, m3u8Base)
            if (qualities.length === 0) qualities = [{ quality: '720p', url: m3u8 }]
            console.log('[AnimeSalt] Qualities found: ' + qualities.map(function(q) { return q.quality }).join(', '))
            return { streams: qualities, subtitle: subtitle, cdnBase: cdnBase }
          })
          .catch(function() {
            return { streams: [{ quality: '720p', url: m3u8 }], subtitle: subtitle, cdnBase: cdnBase }
          })
      })
    })
}
  ...
  return new Promise(function(resolve) {
    ...
    fetch(tmdbUrl)
      ...
      .then(function(streamData) {
        ...
        var subtitles = streamData.subtitle ? [{ url: streamData.subtitle, lang: 'en', name: 'English' }] : []
        resolve(streamData.streams.map(function(s) {
          return {
            name: '🧂 AnimeSalt',
            title: 'AnimeSalt • Multi-Audio • ' + s.quality,
            url: s.url,
            quality: s.quality,
            headers: {
              'Referer': cdnDomain + '/',
